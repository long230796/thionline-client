...
import thionline.client.view.XoaphongthiView.Huy;
import thionline.client.view.XoaphongthiView.Xoaphong;
...
public class KhoaPhongthiView extends JFrame {

	private JPanel contentPane;
	private JTextField tMaphong;

	KhoaPhongthiView current;
	...
	public KhoaPhongthiView(Permission auth, List<PhongthiData> ListP, Proxy proxyService, PhongThiView oldAdView) {
...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		...
		JButton btnNewButton = new JButton("Khóa");
	...
	public boolean kiemtra(String maphong) {
		...
		if (!ctrl.checkExistMaPhong(ListP, maphong)) {
			...
			if (!ctrl.isPhongOpen(ListP, maphong)) {
				JOptionPane.showMessageDialog(null, "Hiện tại phòng này đang khóa");
				correct = false;
			}
	...
	class Xoaphong implements ActionListener {
		public void actionPerformed( ActionEvent e ) {
			...
			if (kiemtra(maphong)) {
				...
				// gui du lieu
				String request = "KhoaPhongThi_" + gson.toJson(pPkg);
				String response = proxy.sender(request);
				
				if (response != null) {
					JOptionPane.showMessageDialog(null, "Khóa phòng thi thành công!");
